...
pub fn build(b: *Build) void {
    ...
    writeTemplate(b, "pytests/mount-small-cache.in", "pytests/mount-small-cache", &.{
        .{ .key = "CMAKE_CURRENT_BINARY_DIR", .value = b.fmt("{s}/zig-out", .{b.build_root.path.?}) },
    });
    ...
    const pytests_config_json = b.addConfigHeader(.{
        .style = .{ .autoconf_at = b.path("pytests/config.json.in") },
        .include_path = "config.json",
    }, .{
        .PROJECT_NAME = project_name,
        .VERSION = project_version,
        .CMAKE_SOURCE_DIR = b.build_root.path.?,
        .CMAKE_CURRENT_BINARY_DIR = b.fmt("{s}/zig-out", .{b.build_root.path.?}),
    ...
    if (rpmzig_verify_pure_zig) {
        tdnf_so_mod.addCMacro("TDNF_RPMZIG_VERIFY_PURE_ZIG", "1");
    }
    ...
    // ----- pytest data (in-tree only — pytests are run from the source
    //       tree against the build output; we no longer ship them as an
    //       installable package).

    // pytests/config.json for in-tree `pytest -v` runs
    b.getInstallStep().dependOn(
        &b.addInstallFileWithDir(
            pytests_config_json.getOutputFile(),
            .{ .custom = "pytests-runtime" },
            "config.json",
        ).step,
    );
